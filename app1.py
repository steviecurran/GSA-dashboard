...
dir1 = "App1/"; dir2 = "~/courses/BI_Udemy/python/maps/" #HOME
#dir1 = "/home/steviecurran/mysite/App1/"; dir2 = "/home/steviecurran/maps/" # pythonanywhere
...
df = pd.read_csv(dir1+'merged_data2.csv');
...
def main_table(data,cur,choice,tosum):
    ...
    df1 = data.sort_values(choice);
    names = dfg[choice].unique(); 
...
def table2_fix(data,cur):
    ...
    for (i,name) in enumerate(names):
        ...
        df_name.reset_index(drop=True, inplace=True)
        df_price = df_price.append(df_name.iloc[0])
    df_M = dfo.merge(df_price,on = 'Product Name');
    df_MT = df_M.merge(dfu,on = 'Product Name');
    df_MT['Total Price'] = round(df_MT['Total Price'],2);
    df_MT = df_MT[['Product Name','Description','Items Sold','Total Price']]; 
    return df_MT
...
def sold(data):
    ...
    for (i,name) in enumerate(names):
        ...
        df_name.reset_index(drop=True, inplace=True); 
        df_name['Items Sold'] = df_name['OrderQuantity'].sum(); 
        df_sum =  df_sum.append(df_name.iloc[0]) 
